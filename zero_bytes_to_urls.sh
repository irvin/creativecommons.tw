...
# 用法: ./zero_bytes_to_urls.sh
# 功能: 找出所有 0 bytes 的檔案並轉換為對應的 URL

ROOT_DIR="./creativecommons.tw"
CACHE_FILE="$ROOT_DIR/hts-cache/new.txt"
ZERO_BYTES_FILES="zero_bytes_files.txt"
ZERO_BYTES_URLS="zero_bytes_urls.txt"

echo "🔍 搜尋 0 bytes 的檔案..."

# 找出所有 0 bytes 的檔案
find "$ROOT_DIR" -type f -size 0 > "$ZERO_BYTES_FILES"

ZERO_COUNT=$(wc -l < "$ZERO_BYTES_FILES")
echo "📁 找到 $ZERO_COUNT 個 0 bytes 的檔案"
...
while IFS= read -r fullpath; do
  ...
  if [ -n "$url" ]; then
    ...
    echo "$url" >> "$ZERO_BYTES_URLS"
  else
    echo "[debug] ❌ 無法對應：$relpath"
  fi
done < "$ZERO_BYTES_FILES"
